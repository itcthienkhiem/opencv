...
def main():
    ...
    #     if dilate[i, i] == 255:
    #         area, rect = cv2.floodFill(dilate, mask, (i,i), 64) # changes on mask
    #         if area > max_area:
    #             max_area = area
    #             point = (i,i)
    ...
    print(rect)
    print max_area
    mask = np.zeros((height+2, width+2), np.uint8) # funzione di mask????
    dilate = dilateMain.copy()
    cv2.floodFill(dilate, mask, point, 64)
    cv2.imshow('x',cv2.absdiff(cv2.dilate(thresh,kernel), dilate))

    '''
    for r in range(height):
        for c in range(width):
            if dilate[r, c] == 64:
                cv2.floodFill(dilate, mask2, (c,r), 0)
'''
